...
Class Action {
	...
	function save_franchise()
	{
		...
		if (empty($id)) {
			...
			$i = 0;
			while ($i == 0) {
				$fcode = substr(str_shuffle($chars), 0, 15);
				$chk = $this->db->query("SELECT * FROM franchise where franchise_code = '$fcode'")->num_rows;
				if ($chk <= 0) {
					$i = 1;
				}
			}
			$data .= ", franchise_code='$fcode' ";
			$save = $this->db->query("INSERT INTO franchise set $data");
		} else {
			$save = $this->db->query("UPDATE franchise set $data where id = $id");
		}
		if ($save) {
			return $fcode;
		}
	...
	{
		extract($_POST);
		$delete = $this->db->query("DELETE FROM franchise where id = $id");
		if ($delete) {
			return 1;
		}
	}
